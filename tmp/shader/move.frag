...
void main() {
    ...
    vec2 p2 = vec2(.25*cos(rad)-1./6.*sin(rad),.25*cos(rad)+1./6.* sin(rad)) + vec2(.5);;
  
    float dist = distance(p1,st);
    float dist2 = distance(p2,st);

    gl_FragColor = vec4(st.x*st.x*.6,0.,abs(sin(10.*dist2*dist*6.*(u_time+200.))),1.0);
}
